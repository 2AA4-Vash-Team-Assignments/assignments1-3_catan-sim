    ...
    protected Command tryHandle(Player agent, CatanGame game, List<Player> allPlayers) {
        ...
        if (agent.canBuildCity()) {
            for (Node node : board.getUpgradeableNodes(agent)) {
                ...
                scores.add(ValueMaximizingHandler.evaluateAction(agent, true, BuildCityCommand.TOTAL_COST));
            }
        }
        if (agent.canBuildSettlement()) {
            for (Node node : board.getAvailableSettlementNodes(agent)) {
                ...
                scores.add(ValueMaximizingHandler.evaluateAction(agent, true, BuildSettlementCommand.TOTAL_COST));
            }
        }
        if (agent.canBuildRoad()) {
            for (Edge edge : board.getAvailableRoadEdges(agent)) {
                options.add(new BuildRoadCommand(agent, edge, game));
                scores.add(ValueMaximizingHandler.evaluateAction(agent, false, BuildRoadCommand.TOTAL_COST));
            }
        }
        if (options.isEmpty()) {
            return null;
        ...
        for (int i = 0; i < scores.size(); i++) {
            if (Double.compare(scores.get(i), best) == 0) {
                bestIndices.add(i);
            }
        }
